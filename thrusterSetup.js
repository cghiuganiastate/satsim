 ...
 * It creates the physics objects, visual representations, and automatically
 * maps them to keyboard controls for translation and rotation.
 *
 * @param {string} configUrl - The URL or path to the thruster configuration JSON file.
 ...
 * @param {function} createThrusterVisual - A function that creates the visual representation of a single thruster.
...
export default async function initializeThrusters(configUrl, CANNON, satMesh, keyToThrusterIndices, createThrusterVisual) {
  try {
    ...
    const thrusters = config.thrusters.map((t, i) => {
      ...
      // --- DATA SANITIZATION ---
      // This is the critical fix. We ensure thrust and isp are valid numbers.
      ...
      // Return the sanitized thruster object
